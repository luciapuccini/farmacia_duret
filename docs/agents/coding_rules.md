...
each Next.js page has all the context to work in isolation

```
app/
    contact/
        page.tsx -> server comp page entry
        page.module.scss --> page styles (may inherit things from global..)
        actions.ts --> server actions for this page ([TBD] good place for zod schemas and validations) "controller"
        components/ --> folder with specific page layouting/components (may inherit/ import from src/components = common)
        tests --> unit integration test for this components or specific flows [TBD]


```

## Typescript convensions

1. I like all react components to have explicit retun type defined = JSX.Element
2. Components props by default should have React's ComponentProps<"element">
3. help me type with React's types abstractions where possible: source = https://github.com/DefinitelyTyped/DefinitelyTyped/blob/master/types/react/index.d.ts
