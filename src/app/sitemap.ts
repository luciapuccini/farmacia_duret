...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const subcategoryRoutes = catalogCategories.flatMap((category) => {
    ...
    return category.subcategories.map((subcategory) =>
      route(subcategoryPath(categorySlug, nameToSlug(subcategory.name)), 'weekly', 0.7),
    );
  });

  const filterRoutes = catalogCategories.flatMap(
    (category) =>
      category.subcategories?.flatMap(
        (subcategory) =>
          subcategory.filters?.map((filter) => route(filter.url, 'weekly', 0.6)) ?? [],
      ) ?? [],
  );
